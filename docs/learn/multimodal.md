...
- Explain the purpose of multimodal prompting.
- Apply the concept to a small prompt.
- Evaluate the result with an explicit rubric.

## Why this matters

Work with images, screenshots, diagrams, charts, video, audio, and ambiguity. Without this layer, prompt advice becomes a list of tricks instead of a system that can be tested.
...
Multimodal prompting connects prompt text to observable behavior. The lesson asks what information the model receives, which authority each piece has, and how the final answer will be checked.
