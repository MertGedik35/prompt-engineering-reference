...
- Explain the purpose of structured outputs.
- Apply the concept to a small prompt.
- Evaluate the result with an explicit rubric.

## Why this matters

Design JSON, JSON Schema, function schemas, validation, and retry loops. Without this layer, prompt advice becomes a list of tricks instead of a system that can be tested.
...
Structured outputs connects prompt text to observable behavior. The lesson asks what information the model receives, which authority each piece has, and how the final answer will be checked.
