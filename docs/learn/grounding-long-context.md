...
- Explain the purpose of grounding and long context.
- Apply the concept to a small prompt.
- Evaluate the result with an explicit rubric.

## Why this matters

Use evidence, source hierarchy, long-document decomposition, and provenance. Without this layer, prompt advice becomes a list of tricks instead of a system that can be tested.
...
Grounding and long context connects prompt text to observable behavior. The lesson asks what information the model receives, which authority each piece has, and how the final answer will be checked.
