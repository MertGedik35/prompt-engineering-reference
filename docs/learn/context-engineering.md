...
- Explain the purpose of context engineering.
- Apply the concept to a small prompt.
- Evaluate the result with an explicit rubric.

## Why this matters

Manage instruction priority, retrieved context, state, memory, and budgets. Without this layer, prompt advice becomes a list of tricks instead of a system that can be tested.
...
Context engineering connects prompt text to observable behavior. The lesson asks what information the model receives, which authority each piece has, and how the final answer will be checked.
