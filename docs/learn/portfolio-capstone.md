...
- Explain the purpose of portfolio and capstone.
- Apply the concept to a small prompt.
- Evaluate the result with an explicit rubric.

## Why this matters

Build reviewable projects that demonstrate prompt design and evaluation skill. Without this layer, prompt advice becomes a list of tricks instead of a system that can be tested.
...
Portfolio and capstone connects prompt text to observable behavior. The lesson asks what information the model receives, which authority each piece has, and how the final answer will be checked.
