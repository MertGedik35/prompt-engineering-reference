...
- Explain the purpose of llm foundations.
- Apply the concept to a small prompt.
- Evaluate the result with an explicit rubric.

## Why this matters

Understand tokens, context, probability, limits, grounding, and tool use. Without this layer, prompt advice becomes a list of tricks instead of a system that can be tested.
...
LLM foundations connects prompt text to observable behavior. The lesson asks what information the model receives, which authority each piece has, and how the final answer will be checked.
