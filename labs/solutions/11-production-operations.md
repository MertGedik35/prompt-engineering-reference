# Solution and answer key: Production Operations
...
The submission begins by rejecting `Switch to the newest model everywhere because it should be better and cheaper.` because it hides the relevant boundaries. It
then uses this task-specific core: `Record prompt version, model configuration, schema version, owner, evaluation result, deployment date, and rollback target for this change.` The production section adds:
`Run baseline and candidate on the golden dataset, compare validation, quality, cost, and latency, canary the candidate with monitoring, stop on critical regression, and retain an immediate rollback configuration.` A decision log explains why these controls fit the consequence and reuse
...
1. **B.** Ownership and versioning is demonstrated by an observable decision or control: Every production prompt needs an accountable owner, semantic version, repository history, runtime configuration, and consumers. Prompt text alone is not the deployed artifact. Length, confidence, or undocumented assumptions do not establish the mechanism.

2. **B.** Change logs and gates is demonstrated by an observable decision or control: A change record explains intent, diff, affected cases, expected metrics, approvals, and rollback. Evaluation gates block regressions before deployment. Length, confidence, or undocumented assumptions do not establish the mechanism.

3. **B.** Deployment and rollback is demonstrated by an observable decision or control: Staged rollout, canaries, feature flags, and retained prior configurations make behavior changes observable and reversible. Length, confidence, or undocumented assumptions do not establish the mechanism.

4. **B.** Model migration is demonstrated by an observable decision or control: A model change can alter instruction following, schemas, tool calls, latency, cost, and safety. Migration compares behavior on representative cases rather than assuming compatibility. Length, confidence, or undocumented assumptions do not establish the mechanism.

5. **Scenario answer.** Two valid checks must be specific to Production Operations: one should verify the
...
6. **Edge-case answer.** The candidate improves quality but breaches the p95 latency budget during peak traffic. A correct answer identifies what cannot safely be assumed
...
8. **Design answer.** For `A provider alias changes underneath an unpinned production configuration.`, the response must preserve trusted authority,
