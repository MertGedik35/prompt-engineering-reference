# Solution and answer key: Portfolio and Capstone
...
The submission begins by rejecting `I engineered a perfect prompt that makes any model 80% more accurate.` because it hides the relevant boundaries. It
then uses this task-specific core: `Write a case-study outline containing problem, users, constraints, prompt versions, six test cases, results, limitations, and links to reproducible artifacts.` The production section adds:
`Prepare a redacted portfolio package with versioned prompts, documented data provenance, evaluation methodology, uncertainty, failure analysis, privacy review, reproducibility instructions, and claims limited to measured evidence.` A decision log explains why these controls fit the consequence and reuse
...
1. **B.** Project selection is demonstrated by an observable decision or control: A useful capstone has a real user, bounded task, available non-sensitive data, observable success, and enough complexity to demonstrate judgment. Length, confidence, or undocumented assumptions do not establish the mechanism.

2. **B.** Problem statements is demonstrated by an observable decision or control: Describe the baseline problem, stakeholders, constraints, consequence of error, and what is explicitly outside scope. Length, confidence, or undocumented assumptions do not establish the mechanism.

3. **B.** Evidence design is demonstrated by an observable decision or control: Artifacts should include prompt versions, representative cases, source or schema contracts, evaluation code, results, and decisions caused by evidence. Length, confidence, or undocumented assumptions do not establish the mechanism.

4. **B.** Evaluation reporting is demonstrated by an observable decision or control: Report dataset composition, metrics, rubric anchors, reviewer process, uncertainty, and regressions. A single polished example is not a benchmark. Length, confidence, or undocumented assumptions do not establish the mechanism.

5. **Scenario answer.** Two valid checks must be specific to Portfolio and Capstone: one should verify the
...
6. **Edge-case answer.** The project has strong results on six synthetic English cases but no production traffic or cross-model testing. A correct answer identifies what cannot safely be assumed
...
8. **Design answer.** For `A portfolio draft implies a course directory is an earned certification and removes failed cases from the reported dataset.`, the response must preserve trusted authority,
